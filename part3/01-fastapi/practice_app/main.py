...
from practice_app.model import MyEfficientNet, get_model, get_config, predict_from_image_byte
...
class Product(BaseModel):
    ...
    # default_factory: Product Class가 처음 만들어질 때 호출되는 함수를 uuid4로 하겠다 
...
class Order(BaseModel):
    ...
    def add_product(self, product: Product):
        # Product를 인자로 받아서, 해당 id가 이미 존재하는지 체크 -> 없다면 products 필드에 추가
        # updated_at을 현재 시각으로 업데이트
...
orders = []
# 실무에서는 보통 이 경우에 DB를 이용해서 주문을 저장하지만, 우리는 in memory인 리스트에 저장
...
async def get_order(order_id: UUID) -> Union[Order, dict]:
    ...
    # 만약 get_order_by_id에서 아무런 데이터가 없어 빈 리스트가 나오면?
    if not order:
...
def get_order_by_id(order_id: UUID) -> Optional[Order]:
    ...
    # generator
    # iter, next 키워드로 검색
    # generator를 사용한 이유 : 메모리를 더 절약할 수 있음
    # iterator, iterable, generator : 파이썬 면접에서 많이 나오는 소재. GIL.
    # iter는 반복 가능한 객체에서 iterator를 반환
    # next는 iterator에서 값을 차례대로 꺼냄
